...
const {Message, StringSession, Image, Video} = require('./𝙍𝘼𝙃𝙀𝙀𝙎/');
const { DataTypes } = require('sequelize');
const { GreetingsDB, getMessage } = require("./plugins/sql/greetings");
const got = require('got');
const 𝙍𝘼𝙃𝙀𝙀𝙎Stack = require('𝙍𝘼𝙃𝙀𝙀𝙎-npm');
...
// Sql
const 𝙍𝘼𝙃𝙀𝙀𝙎DB = config.DATABASE.define('𝙍𝘼𝙃𝙀𝙀𝙎Duplicated', {
    info: {
...
async function 𝙍𝘼𝙃𝙀𝙀𝙎 () {
    ...
    clh.exc_pl = exc_fn
    clh.pay = ddd
    const 𝙍𝘼𝙃𝙀𝙀𝙎CN = new WAConnection();
    const Session = new StringSession();
    try {
      𝙍𝘼𝙃𝙀𝙀𝙎CN.version = [3, 3234, 9]
    } catch {
      console.log(`passed v${𝙍𝘼𝙃𝙀𝙀𝙎CN.version}`)
    }
    𝙍𝘼𝙃𝙀𝙀𝙎CN.setMaxListeners(0);
    var proxyAgent_var = ''
    if (config.PROXY.includes('https') || config.PROXY.includes('http')) {
      𝙍𝘼𝙃𝙀𝙀𝙎CN.connectOptions.agent = ProxyAgent (config.PROXY)
    }
    setInterval(async () => { 
        var getGMTh = new Date().getHours()
        var getGMTm = new Date().getMinutes()
        var ann_msg = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.daily_announcement(config.LANG)
        var ann = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.ann()
        while (getGMTh == 19 && getGMTm == 1) {
            ...
            if (ann.video.includes('http') || ann.video.includes('https')) {
                ...
                yt.on('end', async () => {
                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid,fs.readFileSync('./' + VID + '.mp4'), MessageType.video, {caption: ilan + ann_msg.replace('{user}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.name).replace('{wa_version}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.os_version).replace('{device_model}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.device_model).replace('{device_brand}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.device_manufacturer), mimetype: Mimetype.mp4});
                });
            } else {
                if (ann.image.includes('http') || ann.image.includes('https')) {
                    var imagegen = await axios.get(ann.image, { responseType: 'arraybuffer'})
                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, Buffer.from(imagegen.data), MessageType.image, { caption: ilan + ann_msg.replace('{user}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.name).replace('{wa_version}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.os_version).replace('{device_model}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.device_model).replace('{device_brand}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.device_manufacturer)})
                } else {
                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, ilan + ann_msg.replace('{user}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.name).replace('{wa_version}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.os_version).replace('{device_model}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.device_model).replace('{device_brand}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.phone.device_manufacturer), MessageType.text)
                }
            }
        }
    }, 50000);
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            var timezone_bio = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.timezone(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid)
            var date_bio = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.datebio(config.LANG)
            const biography = '📅 ' + date_bio + '\n⌚ ' + timezone_bio
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.setStatus(biography)
        }
    }, 7890);
    var shs1 = ''
    var shl2 = ''
    ...
    var StrSes_Db = await 𝙍𝘼𝙃𝙀𝙀𝙎DB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    𝙍𝘼𝙃𝙀𝙀𝙎CN.logger.level = logger_levels
    var nodb;
    if (StrSes_Db.length < 1) {
        nodb = true;
        𝙍𝘼𝙃𝙀𝙀𝙎CN.loadAuthInfo(Session.deCrypt(config.SESSION)); 
    } else {
        𝙍𝘼𝙃𝙀𝙀𝙎CN.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }
    𝙍𝘼𝙃𝙀𝙀𝙎CN.on('open', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login Information Updated!')
        );
        const authInfo = 𝙍𝘼𝙃𝙀𝙀𝙎CN.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
            await 𝙍𝘼𝙃𝙀𝙀𝙎DB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
            await StrSes_Db[0].update({ value: Session.createStringSession(authInfo) });
        }
    })    
    𝙍𝘼𝙃𝙀𝙀𝙎CN.on('connecting', async () => {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    𝙍𝘼𝙃𝙀𝙀𝙎CN.on('open', async () => {
        ...
        console.log(chalk.bgGreen('🐺 𝙍𝘼𝙃𝙀𝙀𝙎' + afwhasena));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*𝙍𝘼𝙃𝙀𝙀𝙎 Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*𝙍𝘼𝙃𝙀𝙀𝙎 Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*𝙍𝘼𝙃𝙀𝙀𝙎 Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using 𝙍𝘼𝙃𝙀𝙀𝙎 💌*\n    *- Eva*'
        if (𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == one || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == two || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == three || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == four || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == five || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == six || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == seven || 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid == eight) {
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid,nw, MessageType.text), console.log(nw), await new Promise(r => setTimeout(r, 1000))
            await heroku.get(baseURI + '/formation').then(async (formation) => { 
                forID = formation[0].id; 
                await heroku.patch(baseURI + '/formation/' + forID, { 
                    body: { 
        ...
        if (config.FULLEVA == 'true') {
            var eva_msg = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.eva_if(config.LANG)
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, eva_msg, MessageType.text)
        }
        else {
            var af_start = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.work_type(config.WORKTYPE, config.LANG)
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, af_start, MessageType.text)
        }
        await git.fetch();
        var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
        if (commits.total === 0) {
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(
                𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid,
            ...
            var up_ch = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.update(config.LANG)
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, up_ch, MessageType.text)
        }
    })
    𝙍𝘼𝙃𝙀𝙀𝙎CN.on("chat-update", async (m) => {
        ...
        if (config.NO_ONLINE) {
            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.updatePresence(msg.key.remoteJid, Presence.unavailable);
        }
        // ==================== Greetings ====================
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                if (gb.message.includes('{gpp}')) {
                    var ppUrl = await 𝙍𝘼𝙃𝙀𝙀𝙎CN.getProfilePicture(msg.key.remoteJid) 
                    var nwjson = await 𝙍𝘼𝙃𝙀𝙀𝙎CN.groupMetadata(msg.key.remoteJid)
                    const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
                    await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(msg.key.remoteJid, Buffer.from(resim.data), MessageType.image, { mimetype: Mimetype.png, caption: gb.message.replace('{gpp}', '').replace('{botowner}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.name).replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc) });
                } else {
                    var nwjson = await 𝙍𝘼𝙃𝙀𝙀𝙎CN.groupMetadata(msg.key.remoteJid)
                    await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(msg.key.remoteJid, gb.message.replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc).replace('{botowner}', 𝙍𝘼𝙃𝙀𝙀𝙎CN.user.name), MessageType.text);
                }
            }
            return;
        ...
        if (config.SUPPORT == '917994206751-1612300121') {     
            var sup = config.SUPPORT.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sup.includes(msg.key.remoteJid.split('@')[0]) : sup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (config.SUPPORT2 == '917994206751-1617736751') {     
            var tsup = config.SUPPORT2.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (config.SUPPORT3 == '917994206751-1621015274') {     
            var nsup = config.SUPPORT3.split(',');                            
            if(msg.key.remoteJid.includes('-') ? nsup.includes(msg.key.remoteJid.split('@')[0]) : nsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (config.SUPPORT4 == '917994206751-1625319286') {     
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = 𝙍𝘼𝙃𝙀𝙀𝙎CN.chats.get(msg.key.remoteJid)
                    ...
                    if ((OWN.ff == "917994206751,0" && msg.key.fromMe === false && command.fromMe === true &&
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.chatRead(msg.key.remoteJid);
                        }
                        var match = text_msg.match(command.pattern);
                        if (command.on !== undefined && (command.on === 'image' || command.on === 'photo' )
                        && msg.message.imageMessage !== null) {
                            whats = new Image(𝙍𝘼𝙃𝙀𝙀𝙎CN, msg);
                        } else if (command.on !== undefined && (command.on === 'video')
                        && msg.message.videoMessage !== null) {
                            whats = new Video(𝙍𝘼𝙃𝙀𝙀𝙎CN, msg);
                        } else {
                            whats = new Message(𝙍𝘼𝙃𝙀𝙀𝙎CN, msg);
                        }
                        ...
                        try {
                            ...
                            var error_report = await 𝙍𝘼𝙃𝙀𝙀𝙎Stack.error(config.LANG)
                            await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, error_report.replace('{real_error}', error), MessageType.text, {detectLinks: false})

                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                if (error.message.includes('URL')) {
                                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, '*⚕️ HATA ÇÖZÜMLEME [𝙍𝘼𝙃𝙀𝙀𝙎] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                    ...
                                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, '*🙇🏻 Maalesef Bu Hatayı Okuyamadım! 🙇🏻*' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, '*⚕️ ERROR ANALYSIS [𝙍𝘼𝙃𝙀𝙀𝙎] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await 𝙍𝘼𝙃𝙀𝙀𝙎CN.sendMessage(𝙍𝘼𝙃𝙀𝙀𝙎CN.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
